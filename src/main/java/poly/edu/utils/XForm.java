...
public class XForm {
	...
	*/
	...
	return RRShare.request().getParameter(name) != null;
	}
	/**
	...
	*/
	...
	String value = RRShare.request().getParameter(name);
	...
	public static double getDouble(String name, double defval) {
		...
		*/
		public static File save(String name, String folder) {
			ServletContext context = RRShare.request().getServletContext();
		...
		try {
		Part part = (Part) RRShare.request().getPart(name);
		if(part != null && part.getSize() > 0) {
		String fn = System.currentTimeMillis() + 
		((javax.servlet.http.Part) part).getSubmittedFileName();
		String filename = Integer.toHexString(fn.hashCode()) + 
				fn.substring(fn.lastIndexOf("."));
				File file = new File(dir, filename);
				((javax.servlet.http.Part) part).write(file.getAbsolutePath());
				return file;
				}
				return null;
				} catch (Exception e) {
				...
				*/
				...
				BeanUtils.populate(bean, RRShare.request().getParameterMap());
				return bean;
				} catch (Exception e) {
				throw new RuntimeException(e);
				}
